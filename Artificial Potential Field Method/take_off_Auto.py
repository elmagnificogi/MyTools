...
while True :
    ...
    if len(wait_obj_list) >0:
        ...
        for s in wait_obj_list:
            dis = (s.pos[0] - s.target[0]) ** 2 + (s.pos[1] - s.target[1]) ** 2 + (s.pos[2] - s.target[2]) ** 2
            dis = dis ** 0.5
        ...
        for s in new_wait_obj_list:
            pos1 = s.pos
            collision_c = False
            if len(update_obj_list) == takeoff_num:
                break
    ...
    for sq in update_obj_list:
        ...
        Fd[0] = -sq.speed[0] * (max_a/max_v_xy)
        Fd[1] = -sq.speed[1] * (max_a/max_v_xy)
        Fd[2] = -sq.speed[2] * (max_a/max_v_z)
        #print 'Fd',Fd
               
        # calculate the attractive force
        Fa = [0.0,0.0,0.0]
        Fa[0] = Attr_force * (sq.target[0] - sq.pos[0]) / dis *sq.mass
        Fa[1] = Attr_force * (sq.target[1] - sq.pos[1]) / dis *sq.mass
        Fa[2] = Attr_force * (sq.target[2] - sq.pos[2]) / dis *sq.mass
            ...
            if sq2 != sq:
                ...
                if b_dis < view_radius and b_dis >=slow_down_dis:
                    ...
                    if (theta < 2*math.pi) and (math.fabs(theta) < safe_angle):# or (math.pi - theta) < safe_angle):
                        ...
                        Fr[2] += Fdef[2]
                        if sq2.arrived:
                            Fr[0] +=  -math.fabs(Repu_force) * (pos[0] - sq.pos[0]) / b_dis / b_dis * 0.0
                            Fr[1] +=  -math.fabs(Repu_force) * (pos[1] - sq.pos[1]) / b_dis / b_dis * 0.0
                            Fr[2] +=  -math.fabs(Repu_force) * (pos[2] - sq.pos[2]) / b_dis / b_dis * 0.0
                        else:
        ...
        if sq.pos[2] + sq.speed[2] * update_time > takeoff_height+ground_height:
            # in normal mp situation
            sq.state = 2
            sq.pos[0] = sq.pos[0] + sq.speed[0] * update_time
            sq.pos[1] = sq.pos[1] + sq.speed[1] * update_time
            sq.pos[2] = sq.pos[2] + sq.speed[2] * update_time
        elif sq.state==1:
            # just takeoff need to go higher
            sq.pos[2]  += speed_ver_max * update_time 
            sq.speed[0] = 0.0
            sq.speed[1] = 0.0
            sq.speed[2] = speed_ver_max
        ...
        if dis < sq.max_dis * dis_threshold:
            ...
            all_arrived +=1
            sq.mass = 0.7
            #print 'Not arrived:',takeoff_num-all_arrived
            if maya_show:
                cmds.progressWindow(edit=True, progress = int((all_arrived+len(update_obj_list))* 50.0 / len(spheres)))
...
if cur_frame <= overtime:
    if maya_show:
        cmds.intField("Dmd_UAVC_Motion_start_time", edit=True,value =1)
        cmds.intField("Dmd_UAVC_Motion_end_time", edit=True,value =cur_frame+1)
